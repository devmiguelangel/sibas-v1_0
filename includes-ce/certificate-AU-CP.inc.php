   ...
   <div id="container-cert" style="<?=$width_ct2;?> font-weight: normal; font-size: 80%; font-family: Arial, Helvetica, sans-serif; color: #000000;">
     ...
     <table border="0" cellpadding="0" cellspacing="0" style="width: 100%; <?=$fontSize;?>">
         ...
         <tr>
           ...
           <td style="width:50%;"><?=$row['plazo'];?></td>
         </tr>
         <!--<tr>
         ...
         <tr>
           ...
           <td style="width:50%;"><?=number_format(($row['prima_total']*$row['c_tipo_plazo']),2,".",",");?> USD</td>
         </tr>
         <tr>
...
<?php
            ...
            while($rowDt = $rsDt->fetch_array(MYSQLI_ASSOC)){
                $year = $link->get_year_final($row['c_plazo'], $row['c_tipo_plazo']);
                $primaVh = 0;
                $primaT = 0;
                $tasaT = 0;

                if(($rowTasa = $link->get_tasa_year_au(base64_encode($row['idcia']),
                        base64_encode($row['idef']), $rowDt['vh_categoria'],
                        $year, $row['c_forma_pago'])) !== FALSE){
                    $tasaT = $rowTasa['t_tasa_final'];
                    $primaT = ($rowDt['valor_asegurado'] * $tasaT) / 100;
                }
			?>
                <tr>
                ...
                <td style="width:10%; text-align:center;"><?=$tasaT;?></td>
                <td style="width:10%; text-align:center;"><?=$primaT;?></td>
                </tr> 
            <?php
